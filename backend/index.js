...
import http from 'http'; // NEW: Import the http module for Socket.IO
...
import connectionsRoutes from './routes/connections.js'; // Connection routes for the new feature
...
// The connections routes already have their own verifyToken middleware, so we don't need it here.
...
        id: messageDoc.id,
        ...messageData,
        createdAt: messageData.createdAt?.toDate().toISOString() // Optional chaining for safety
      });
    } catch (error) {
...
io.on('connection', (socket) => {
    ...
    // Join a room for one-on-one chat
    socket.on('join_chat', (chatId) => {
        socket.join(chatId);
        console.log(`User ${socket.id} joined chat room: ${chatId}`);
    });

    // NEW: Join a room for group chat
    socket.on('join_group_chat', (groupId) => {
        socket.join(groupId);
        console.log(`User ${socket.id} joined group room: ${groupId}`);
    });

    // Handle sending and receiving messages (no changes needed, it will work for groups too)
    // The `chatId` will now be the `groupId` for group messages
    socket.on('send_message', async (data) => {
        const { chatId, senderId, text } = data; // Note: RecipientId is not needed for groups
        const db = admin.firestore();
