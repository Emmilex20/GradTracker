...
import { FaPaperPlane, FaVideo, FaArrowDown, FaUsers, FaArrowLeft, FaDollarSign } from 'react-icons/fa';
...
interface Message {
    ...
    senderName?: string;
    createdAt: Date;
    type?: string; // Add a 'type' field to distinguish messages
    requestData?: { // Add this for financial support requests
...
const GroupChatPage: React.FC = () => {
    // ... (rest of the state and hooks are the same)
    ...
    return (
        <div className="min-h-screen w-full mt-8 bg-slate-950 text-white flex justify-center items-center py-16 px-4 relative overflow-hidden">
            {/* Background Blobs */}
            ...
            <div className="relative z-10 w-full max-w-2xl mx-auto backdrop-blur-3xl bg-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[80vh] md:h-[90vh]">
                ...
                <div className="p-4 md:p-6 bg-white/5 border-b border-white/10 flex items-center justify-between space-x-4 animate-fade-in-down">
                    ...
                    <button
                        ...
                        className="bg-green-500 text-white px-4 py-2 rounded-full flex items-center space-x-2 hover:bg-green-600 transition-all transform hover:scale-105"
                    >
                        <FaVideo />
                ...
                >
                    {messages.length > 0 ? (
                        messages.map((msg) => (
                            ...
                            >
                                {/* NEW: Conditional rendering based on message type */}
                                {msg.type === 'financial_support_request' ? (
                                    <div className="bg-fuchsia-600/60 p-4 rounded-xl max-w-[80%] md:max-w-[70%] text-sm md:text-base shadow-lg transition-all duration-300 transform hover:scale-[1.02]">
                                        <div className="flex items-center space-x-2 mb-2">
                                            <FaDollarSign className="text-white" />
                                            <span className="font-bold text-white">Financial Support Request</span>
                                        </div>
                                        <p className="text-gray-100">
                                            <span className="font-semibold">{msg.senderName}</span> has requested financial support for {msg.requestData?.universityName}.
                                        </p>
                                        <p className="text-xs text-gray-200 mt-1">Requested Amount: ${msg.requestData?.requestedAmount}</p>
                                        {msg.requestData?.notes && (
                                            <p className="text-xs text-gray-300 italic mt-1">Note: "{msg.requestData.notes}"</p>
                                        )}
                                    </div>
                                ) : (
                                    <div
