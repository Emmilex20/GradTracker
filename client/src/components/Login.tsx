...
const Login: React.FC = () => {
  ...
  const handleEmailPasswordLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('⚠️ Failed to log in: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError('⚠️ Failed to log in with Google: ' + err.message);
    } finally {
  ...
  return (
    <div className="min-h-screen flex bg-neutral-100 py-20">
      {/* Left-side banner - large screens only */}
            ...
            >
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  <AiOutlineLoading3Quarters className="animate-spin" size={20} /> Logging in...
                </span>
              ) : (
                'Log In'
              )}
