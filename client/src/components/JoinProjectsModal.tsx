...
  pendingRequests: string[]; // Add pendingRequests to the interface
}
...
          } else if (project.pendingRequests.includes(currentUser?.uid as string)) {
...
      // Optionally re-fetch projects to update the list
...
          className="bg-primary text-white font-semibold py-2 px-4 rounded-full transition-all duration-300 hover:bg-indigo-700 transform hover:scale-105 flex items-center space-x-2"
        >
          <FaSignInAlt />
...
    const className = status === 'requested'
      ? `${baseClass} bg-neutral-400 text-white cursor-not-allowed`
      : `${baseClass} bg-primary text-white hover:bg-indigo-700 transform hover:scale-105`;
    
    const buttonText = status === 'requested' ? 'Requested' : 'Join';
